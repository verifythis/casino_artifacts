...
AddToPot(sender, money) ==
    ...
    /\ money > 0
    /\ pot' = pot + money
    /\ WALLETS' = [ WALLETS EXCEPT ![operator] = WALLETS[operator] - money ] 
...
RemoveFromPot(sender, amount) ==
    ...
    /\ WALLETS' = [ WALLETS EXCEPT ![operator] = WALLETS[operator] + amount ] 
    /\ pot' = pot - amount
...
DecideBetWin(sender, secret) ==
    ...
    /\ \* player wins, gets back twice her bet
        pot' = pot - bet
    /\ WALLETS' =   [ WALLETS EXCEPT ![player] = WALLETS[player] - 2*bet ]
    /\ bet = 0
    /\ state' = "IDLE"
...
Spec == \forall op \in Int : 
        \forall secret \in Int :
          /\ Init(op)
          /\ [][Step(secret)]_<<>>
===============================================================================
